...
class DashboardIntegrationTest extends AbstractIntegrationTest {
    ...
    void setUp() {
        ...
        jdbcClient.sql(
                "INSERT INTO companies (id, name, slug, organization_id, tax_id, is_active) VALUES (?, ?, ?, ?, ?, ?)")
        ...
        employeeId = jdbcClient.sql(
                "INSERT INTO employees (username, password_hash, full_name, organization_id, is_active, created_at, updated_at) VALUES ('testuser', ?, 'Test User', ?, true, NOW(), NOW()) RETURNING id")
                .params(passwordHash, orgId).query(Long.class).single();
