...
class PublicPreferencesControllerIntegrationTest extends AbstractIntegrationTest {
    ...
    void testGetPreferences_Success() throws Exception {
        mockMvc.perform(get("/api/v1/public/marketing/preferences").param("token", "valid-token-123"))
                .andExpect(status().isOk());
    }

    @Test
    void testGetPreferences_InvalidToken() throws Exception {
        mockMvc.perform(get("/api/v1/public/marketing/preferences").param("token", "invalid-token"))
    ...
    void testUpdatePreferences_Success() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/public/marketing/preferences").contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request))).andExpect(status().isOk());
    }

    @Test
    void testUnsubscribeOneClick_Success() throws Exception {
        mockMvc.perform(post("/api/v1/public/marketing/unsubscribe-one-click").param("token", "valid-token-123"))
                .andExpect(status().isOk());
    }
}
