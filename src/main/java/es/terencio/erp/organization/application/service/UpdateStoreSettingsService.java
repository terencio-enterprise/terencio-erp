...
public class UpdateStoreSettingsService implements UpdateStoreSettingsUseCase {
    ...
    public void execute(UpdateStoreSettingsCommand command) {
        ...
        StoreSettings settings = storeSettingsRepository.findByStoreId(storeId).orElseThrow(() -> new DomainException("Store settings not found"));
        Money requireCustomerAmount = command.requireCustomerForLargeAmount() != null ? Money.ofEuros(command.requireCustomerForLargeAmount()) : Money.ofEuros(1000.0);
