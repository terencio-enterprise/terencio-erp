...
public class JdbcCustomerPersistenceAdapter implements CustomerRepository {
    ...
    private Customer insert(Customer customer) {
        ...
        jdbcClient
                ...
                .param("type", customer.getType() != null ? customer.getType().name() : null)
                .param("origin", customer.getOrigin())
                .param("tags", customer.getTags().toArray(new String[0]))
                .param("marketingConsent", customer.isMarketingConsent())
                .param("marketingStatus",
                        customer.getMarketingStatus() != null ? customer.getMarketingStatus().name() : null)
        ...
        Customer newCustomer = new Customer(
                new CustomerId(generatedId), uuid, customer.companyId(), customer.taxId(), customer.legalName(),
                customer.commercialName(), customer.email(), customer.phone(), customer.address(), customer.zipCode(),
                customer.city(), customer.country(), customer.tariffId(), customer.allowCredit(),
                customer.creditLimit(), customer.surchargeApply(), customer.notes(), customer.isActive(),
        ...
        jdbcClient
                .sql("""
                        UPDATE customers SET
                            tax_id = :taxId, legal_name = :legalName, commercial_name = :commercialName, email = :email,
                            phone = :phone, address = :address, zip_code = :zipCode, city = :city, tariff_id = :tariffId,
                            allow_credit = :allowCredit, credit_limit = :creditLimit, surcharge_apply = :surchargeApply,
                            notes = :notes, active = :active, updated_at = :updatedAt, type = :type, origin = :origin,
                            tags = :tags, marketing_consent = :marketingConsent, marketing_status = :marketingStatus,
                            unsubscribe_token = :unsubscribeToken, last_interaction_at = :lastInteractionAt, snoozed_until = :snoozedUntil, deleted_at = :deletedAt
                        WHERE id = :id
                        """)
                ...
                .param("snoozedUntil",
                        customer.getSnoozedUntil() != null ? Timestamp.from(customer.getSnoozedUntil()) : null)
                .param("deletedAt", customer.deletedAt() != null ? Timestamp.from(customer.deletedAt()) : null)
                .update();
    ...
    public Optional<Customer> findById(CustomerId id) {
        return jdbcClient.sql("SELECT * FROM customers WHERE id = :id AND deleted_at IS NULL")
                .param("id", id.value()).query(this::mapRow).optional();
    }
    ...
    public Optional<Customer> findByUuid(UUID uuid) {
        return jdbcClient.sql("SELECT * FROM customers WHERE uuid = :uuid AND deleted_at IS NULL")
                .param("uuid", uuid).query(this::mapRow).optional();
    }

    @Override
    public List<Customer> findByCompanyId(CompanyId companyId) {
        return jdbcClient.sql(
                "SELECT * FROM customers WHERE company_id = :companyId AND active = TRUE AND deleted_at IS NULL ORDER BY legal_name")
                .param("companyId", companyId.value()).query(this::mapRow).list();
    }
    ...
    public List<Customer> searchCustomers(CompanyId companyId, String searchTerm) {
        return jdbcClient.sql("""
                ...
                WHERE company_id = :companyId AND active = TRUE AND deleted_at IS NULL
                AND (LOWER(legal_name) LIKE LOWER(:search) OR tax_id LIKE :search OR phone LIKE :search)
                ORDER BY legal_name LIMIT 50
                """)
    ...
    public List<Customer> findByMarketingCriteria(List<String> tags, String customerType,
        ...
        StringBuilder sql = new StringBuilder("SELECT * FROM customers WHERE active = TRUE AND deleted_at IS NULL");
    ...
    public Optional<Customer> findByUnsubscribeToken(String token) {
        return jdbcClient.sql("SELECT * FROM customers WHERE unsubscribe_token = :token AND deleted_at IS NULL")
                .param("token", token).query(this::mapRow).optional();
    }
    ...
    private Customer mapRow(ResultSet rs, int rowNum) throws SQLException {
        ...
        Customer c = new Customer(
                new CustomerId(rs.getLong("id")), (UUID) rs.getObject("uuid"),
                new CompanyId((UUID) rs.getObject("company_id")),
                taxIdStr != null ? TaxId.of(taxIdStr) : null, rs.getString("legal_name"),
                rs.getString("commercial_name"),
                emailStr != null ? Email.of(emailStr) : null, rs.getString("phone"), rs.getString("address"),
                rs.getString("zip_code"),
                rs.getString("city"), rs.getString("country"), (Long) rs.getObject("tariff_id"),
                rs.getBoolean("allow_credit"),
                Money.ofEurosCents(rs.getLong("credit_limit")), rs.getBoolean("surcharge_apply"), rs.getString("notes"),
                rs.getBoolean("active"), rs.getTimestamp("created_at").toInstant(),
                rs.getTimestamp("updated_at").toInstant(),
                deletedAtTs != null ? deletedAtTs.toInstant() : null);

        c.setType(rs.getString("type") != null ? CustomerType.valueOf(rs.getString("type")) : null);
        c.setOrigin(rs.getString("origin"));
        java.sql.Array tagsArray = rs.getArray("tags");
        if (tagsArray != null) {
            c.setTags(Arrays.asList((String[]) tagsArray.getArray()));
        }
        c.setMarketingConsent(rs.getBoolean("marketing_consent"));
        c.setMarketingStatus(
                rs.getString("marketing_status") != null ? MarketingStatus.valueOf(rs.getString("marketing_status"))
                        : null);
