...
public class JdbcCampaignRepository implements CampaignRepositoryPort {
    ...
    public List<MarketingTemplate> findAllTemplates(UUID companyId, String search) {
        ...
        return jdbcTemplate.query(sql, new MapSqlParameterSource("search", search)
                .addValue("searchPattern", searchPattern).addValue("companyId", companyId), new TemplateRowMapper());
    }
    ...
    public List<CampaignLog> findLogsByStatus(String status) {
        ...
        return jdbcTemplate.query(sql, new MapSqlParameterSource("status", status),
                (rs, i) -> new CampaignLog(
                        rs.getLong("id"), rs.getObject("company_id", UUID.class), rs.getLong("customer_id"),
                        rs.getLong("template_id"), rs.getTimestamp("sent_at").toInstant(),
    ...
    private static class TemplateRowMapper implements RowMapper<MarketingTemplate> {
        ...
        public MarketingTemplate mapRow(ResultSet rs, int rowNum) throws SQLException {
            return new MarketingTemplate(
                    rs.getLong("id"), rs.getObject("company_id", UUID.class), rs.getString("code"),
                    rs.getString("name"),
                    rs.getString("subject_template"), rs.getString("body_html"), rs.getBoolean("is_active"),
