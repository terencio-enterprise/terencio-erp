...
public class CrmCustomerIntegrationAdapter implements CustomerIntegrationPort {
    ...
    public Optional<MarketingCustomer> findByToken(String token) {
        String sql = "SELECT id, email, COALESCE(commercial_name, legal_name, 'Customer') as name, " +
                     "marketing_status, unsubscribe_token, active " +
                     "FROM customers WHERE unsubscribe_token = ? AND deleted_at IS NULL";
    ...
    public void updateMarketingStatus(String token, MarketingStatus status, Instant snoozeUntil) {
        String sql = "UPDATE customers SET marketing_status = ?, updated_at = NOW() WHERE unsubscribe_token = ?";
        jdbcTemplate.update(sql, status.name(), token);
    }
    ...
    private MarketingCustomer mapRow(ResultSet rs, int rowNum) throws SQLException {
        ...
        MarketingStatus status = statusStr != null ? MarketingStatus.valueOf(statusStr) : MarketingStatus.UNSUBSCRIBED;
