...
public class CrmCustomerIntegrationAdapter implements CustomerIntegrationPort {
    ...
    public List<MarketingCustomer> findAudience(AudienceFilter filter) {
        return jdbcClient.sql("SELECT id, company_id, legal_name, email, unsubscribe_token, marketing_status, marketing_consent FROM customers WHERE email IS NOT NULL AND active = true")
            .query((rs, rowNum) -> mapRow(rs)).list();
    }

    @Override
    public Optional<MarketingCustomer> findByToken(String token) {
        return jdbcClient.sql("SELECT id, company_id, legal_name, email, unsubscribe_token, marketing_status, marketing_consent FROM customers WHERE unsubscribe_token = ?")
            .param(token)
    ...
    public void updateMarketingStatus(String token, String status, Instant snoozedUntil) {
        jdbcClient.sql("UPDATE customers SET marketing_status = ?, snoozed_until = ?, updated_at = NOW() WHERE unsubscribe_token = ?")
            .param(status)
            .param(snoozedUntil != null ? java.sql.Timestamp.from(snoozedUntil) : null)
            .param(token)
            .update();
    }
